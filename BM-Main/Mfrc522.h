...
using namespace HardwareAddresses;

using spi0 = spi::SPI<spi::Mode::m1, spi::ClkRate::clkRateDiv4>;
...
struct MFRC522 {
	...
	static ErrorType mfrc522_request(uint8_t req_mode, uint8_t * tag_type) {
		ErrorType  status;
	...
	static ErrorType mfrc522_to_card(Commands cmd, uint8_t *send_data, uint8_t send_data_len, uint8_t *back_data, uint32_t *back_data_len) {
